...
class SubscribeBloc extends Bloc<SubscribeEvent, SubscribeState> {
  ...
  final ApiRepository repository;
   ...
   SubscribeBloc({this.repository}) : super(const SubscribeState(status: SubscribeStatus.Initial));
  ...
  Stream<SubscribeState> _mapToCheckSubscribe(CheckSubscribe event) async*{
    ...
    if(user.isNullOrEmpty) {
      yield const SubscribeState(status: SubscribeStatus.Success, isLogin: false, isSubscribe: false );
    } else {
      ...
      yield SubscribeState(status: SubscribeStatus.Success, isLogin: true, isSubscribe: isSubscribe);
    }
  }
